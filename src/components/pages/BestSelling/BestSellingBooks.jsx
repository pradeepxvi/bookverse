...
const BestSellingBooks = () => {
  const { bestSellingBooks } = useContext(CreateMainContext);

  // ====================page navigation logic====================
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 5;
  const totalPage = Math.ceil(bestSellingBooks.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const currentPageBooks = bestSellingBooks.slice(
    startIndex,
  ...
  if (bestSellingBooks.length == 0) return <EmptyBooks />;
